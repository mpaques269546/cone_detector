...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768,
        ...
        self.grid_size = (img_size[0] // patch_size, img_size[1] // patch_size) # patch_size[1]
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, return_attention=False): ## to add: return_attention
...
class TIMMVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, residual_indices=[], embed_dim=768,
                 ...
                 act_layer=nn.GELU, window_attn=False, window_size=14, with_cp=False, pretrained=None, num_tokens=1):
        """
        Args:
        ...
        self.num_tokens = num_tokens # no cls_token with windowed attention
        ...
        if self.num_tokens>0:
            self.cls_token = nn.Parameter(torch.zeros(1, self.num_tokens, embed_dim)) #original [1,1,embed_dim]
    ...
    def interpolate_pos_encoding(self, x, w, h):
        npatch = x.shape[1] - self.num_tokens #1
        N = self.pos_embed.shape[1] - self.num_tokens #1
        if npatch == N and w == h:
